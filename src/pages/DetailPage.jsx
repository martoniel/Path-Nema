...
import {
  ...
  Search, Download, BookOpen, ChevronLeft, X,
...
const handleDDxClick = async (diseaseName) => {
  ...
  const handleDownloadPDF = () => {
    ...
    section('Introduction');    y = wrap(disease.introduction || disease.overview || '', lm, y, rm - lm); y += 4
    section('Epidemiology');    y = wrap(disease.epidemiology || '', lm, y, rm - lm); y += 4
    section('Etiology');        y = wrap(disease.etiology || '', lm, y, rm - lm); y += 4
    section('Pathophysiology'); y = wrap(disease.pathophysiology || '', lm, y, rm - lm); y += 4
    section('Signs and Symptoms')
    if (disease.signsAndSymptoms) {
      doc.setFont('helvetica', 'bold'); doc.text('Symptoms:', lm, y); y += 5
      doc.setFont('helvetica', 'normal'); bullet(disease.signsAndSymptoms.symptoms)
      doc.setFont('helvetica', 'bold'); doc.text('Signs:', lm, y); y += 5
      doc.setFont('helvetica', 'normal'); bullet(disease.signsAndSymptoms.signs)
    } else { bullet(disease.symptoms) }
    section('Risk Factors'); bullet(disease.riskFactors)
    section('Diagnosis')
    if (disease.diagnosis) {
      ...
      doc.setFont('helvetica', 'normal'); bullet(disease.diagnosis.investigations)
    }
    ...
    if (disease.treatment) {
      ...
      doc.setFont('helvetica', 'normal'); y = wrap(disease.treatment.immediate || '', lm + 3, y, rm - lm - 3); y += 3
      doc.setFont('helvetica', 'bold'); doc.text('Medical:', lm, y); y += 5
      doc.setFont('helvetica', 'normal'); bullet(disease.treatment.medical)
      doc.setFont('helvetica', 'bold'); doc.text('Surgical:', lm, y); y += 5
      doc.setFont('helvetica', 'normal'); y = wrap(disease.treatment.surgical || '', lm + 3, y, rm - lm - 3); y += 3
    }
    section('Management');   y = wrap(disease.management || '', lm, y, rm - lm); y += 4
    section('Complications'); bullet(disease.complications)
    section('Prognosis');    y = wrap(disease.prognosis || '', lm, y, rm - lm); y += 4
    ...
    <div className="min-h-screen pt-[60px]">
      ...
      {lightbox && (
        <div className="fixed inset-0 z-50 bg-black/90 flex items-center justify-center p-6"
          ...
          <button className="absolute top-5 right-6 text-white hover:text-[#2effc0] transition-colors"
            onClick={() => setLightbox(null)}>
      ...
      <div className="max-w-[1060px] mx-auto px-6 py-6">
        ...
        <button onClick={() => navigate(-1)}
          ...
          <ChevronLeft size={16} strokeWidth={2} /> Back to Diagnostic Results
        </button>
        ...
        <div className="bg-[#0f2318] border border-[#1a3328] rounded-2xl p-8 flex items-start justify-between gap-6 mb-8 flex-wrap">
          <div className="flex-1 min-w-[260px]">
            <div className="flex items-center gap-2 text-[#2effc0] text-[10px] font-bold tracking-[2px] uppercase font-body mb-3">
              <CheckCircle size={13} strokeWidth={2} /> High Confidence Analysis
            </div>
            ...
            <div className="flex gap-3 flex-wrap">
              <button onClick={handleDownloadPDF}
                ...
                <Download size={14} strokeWidth={2} /> Download PDF Report
              </button>
              <button onClick={() => navigate(-1)}
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-body font-bold text-[15px] text-[#e8f5f0] flex items-center gap-2">
              <Microscope size={15} strokeWidth={1.5} className="text-[#2effc0]" />
          ...
          {tabs.map(t => (
            <button key={t.id} onClick={() => setActiveTab(t.id)}
              className={`px-5 py-3 font-body text-[13px] font-semibold whitespace-nowrap transition-all border-b-2 -mb-[2px]
                ${activeTab === t.id ? 'text-[#2effc0] border-[#2effc0]' : 'text-[#4a7a64] border-transparent hover:text-[#7aad96]'}`}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-10">
          <div>
            ...
            {activeTab === 'overview' && (
              <div>
                <Section title="Introduction"><p className="font-body text-sm text-[#7aad96] leading-[1.85]">{disease.introduction || disease.overview}</p></Section>
                <Section title="Epidemiology"><p className="font-body text-sm text-[#7aad96] leading-[1.85]">{disease.epidemiology || 'Epidemiological data not available.'}</p></Section>
                <Section title="Etiology"><p className="font-body text-sm text-[#7aad96] leading-[1.85]">{disease.etiology || 'Etiology data not available.'}</p></Section>
                <Section title="Pathophysiology"><p className="font-body text-sm text-[#7aad96] leading-[1.85]">{disease.pathophysiology || 'Pathophysiology data not available.'}</p></Section>
              </div>
            )}
                ...
                <Section title="Signs and Symptoms">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                    <div className="bg-[#0f2318] border border-[#1a3328] rounded-xl p-5">
                      <h4 className="font-body font-bold text-[14px] text-[#2effc0] mb-3 flex items-center gap-2">
                        <Stethoscope size={14} strokeWidth={1.5} /> Symptoms
                      </h4>
                      <ul className="space-y-2">
                        {(disease.signsAndSymptoms?.symptoms || disease.symptoms || []).map((s, i) => (
                          <li key={i} className="flex items-start gap-2 font-body text-[13px] text-[#7aad96]">
                            <span className="w-1 h-1 rounded-full bg-[#2effc0] flex-shrink-0 mt-2" /> {s}
                          </li>
                        ))}
                      </ul>
                    </div>
                    <div className="bg-[#0f2318] border border-[#1a3328] rounded-xl p-5">
                      <h4 className="font-body font-bold text-[14px] text-[#2effc0] mb-3 flex items-center gap-2">
                        <Microscope size={14} strokeWidth={1.5} /> Clinical Signs
                      </h4>
                      <ul className="space-y-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
                    {(disease.riskFactors || []).map((r, i) => (
                      <div key={i} className="flex items-start gap-3 bg-[#0f2318] border border-[#1a3328] rounded-xl p-4">
                        <ShieldAlert size={16} strokeWidth={1.5} className="text-[#2effc0] flex-shrink-0 mt-0.5" />
                  ...
                  <div className="border border-[#1a3328] rounded-xl overflow-hidden mt-4">
                    {(disease.diagnosis?.investigations || []).map((inv, i, arr) => (
                      <div key={i} className={`flex items-start gap-3 px-5 py-4 font-body text-sm ${i < arr.length - 1 ? 'border-b border-[#1a3328]' : ''}`}>
                        <FlaskConical size={15} strokeWidth={1.5} className="text-[#2effc0] flex-shrink-0 mt-0.5" />
    ...
    : <span className="text-[#4a7a64] text-xs font-body font-semibold px-2 py-1 border border-[#1a3328] rounded-lg group-hover:border-[#1aad82] group-hover:text-[#2effc0] transition-colors">DDx</span>
              ...
              <div>
                {disease.treatment?.immediate && (
                  <div className="bg-[rgba(224,80,80,0.08)] border border-[rgba(224,80,80,0.2)] rounded-xl p-5 mb-8">
                    <h3 className="font-body font-bold text-[14px] text-[#e05050] mb-2 flex items-center gap-2">
                      <AlertCircle size={15} strokeWidth={2} /> Immediate Management
                    </h3>
                  ...
                  <ul className="space-y-3 mt-4">
                    {(disease.treatment?.medical || []).map((m, i) => (
                      <li key={i} className="flex items-start gap-3 bg-[#0f2318] border border-[#1a3328] rounded-xl p-4">
                        <span className="text-[#2effc0] font-bold font-body text-sm flex-shrink-0 w-5 text-center">{i + 1}.</span>
                ...
                <Section title="Complications">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
                    {(disease.complications || []).map((c, i) => (
                      <div key={i} className="flex items-start gap-3 bg-[#0f2318] border border-[#1a3328] rounded-xl p-4">
                        <Zap size={15} strokeWidth={1.5} className="text-[#f0c040] flex-shrink-0 mt-0.5" />
          ...
          <div>
            ...
            <div className="bg-[#0f2318] border border-[#1a3328] rounded-2xl p-5 mb-5">
              <h3 className="font-body font-bold text-[15px] text-[#e8f5f0] mb-4 flex items-center gap-2">
                <Activity size={14} strokeWidth={1.5} className="text-[#2effc0]" /> Severity Assessment
              </h3>
            ...
            {/* Key Findings */}
            <div className="bg-[#0f2318] border border-[#1a3328] rounded-2xl p-5 mb-5">
              <h3 className="font-body font-bold text-[15px] text-[#e8f5f0] mb-3 flex items-center gap-2">
                <ClipboardList size={14} strokeWidth={1.5} className="text-[#2effc0]" /> Key Findings
              </h3>
              <ul className="space-y-2">
                {(disease.symptoms || []).slice(0, 4).map((s, i) => (
                  <li key={i} className="flex items-start gap-2 font-body text-[12px] text-[#7aad96]">
                    <CheckCircle size={12} strokeWidth={2} className="text-[#2effc0] flex-shrink-0 mt-0.5" /> {s}
                  </li>
                ))}
              </ul>
            </div>
            ...
            <div className="bg-[#0f2318] border border-[#1a3328] rounded-2xl p-5 mb-5">
              <h3 className="font-body font-bold text-[15px] text-[#e8f5f0] mb-3 flex items-center gap-2">
                <Video size={14} strokeWidth={1.5} className="text-[#2effc0]" /> Educational Video
              </h3>
              {videoLoading ? (
                ...
                <div>
                  <div className="relative w-full rounded-xl overflow-hidden mb-3" style={{ paddingBottom: '56.25%' }}>
                    <iframe className="absolute top-0 left-0 w-full h-full rounded-xl"
                      src={`https://www.youtube.com/embed/${video.videoId}`} title={video.title}
            ...
            {disease.source && (
              <div className="bg-[#0f2318] border border-[#1a3328] rounded-2xl p-4 mb-5">
                <p className="font-body text-[10px] font-bold tracking-[2px] uppercase text-[#4a7a64] mb-1 flex items-center gap-1.5">
                  <BookOpen size={11} strokeWidth={1.5} /> Knowledge Source
                </p>
            ...
            <div className="bg-[#0f2318] border border-[#1a3328] rounded-2xl p-5 text-center">
              <div className="w-14 h-14 rounded-2xl bg-[rgba(46,255,192,0.1)] border border-[#1aad82] flex items-center justify-center mx-auto mb-3">
                <FileText size={24} strokeWidth={1.5} className="text-[#2effc0]" />
              </div>
              ...
              <button onClick={handleDownloadPDF}
                className="w-full py-3 rounded-xl bg-[#2effc0] text-[#071210] font-bold text-[13px] font-body hover:opacity-85 transition-opacity flex items-center justify-center gap-2">
      ...
      <div className="border-t border-[#1a3328] mt-10 py-6 text-center">
        <p className="font-body text-[12px] text-[#4a7a64]">
          Report generated by PathNema Expert AI. Based on {disease.source || "Khurana's Comprehensive Ophthalmology"} and AI clinical pattern recognition.
        </p>
